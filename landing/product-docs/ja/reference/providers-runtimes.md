...
ローカルモデルは **Provider Settings** の OpenCode 経由で接続します。Ollama、LM Studio、Atomic Chat、llama.cpp のプリセットと、独自の OpenAI 互換エンドポイントがあります。チームを起動する前にモデルを検出してテストしてください。表示されるだけではツール呼び出し能力を確認できません。詳しくは [ランタイム設定](/ja/guide/runtime-setup) を参照してください。
