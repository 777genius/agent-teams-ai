...
Каждый участник команды работает через провайдер-бэкенд. В редакторе команды выберите провайдер (Claude, Codex или OpenCode) и модель для каждого участника. Доступные варианты зависят от готовности рантайма, каталога моделей и доступа к провайдеру. Бесплатные модели OpenCode могут быть доступны без авторизации.
