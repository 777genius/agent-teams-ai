...
export type ProviderAccountAvailability = "available" | "limited" | "reconnect_required" | "auth_unknown" | "unhealthy" | "unknown";
export type ProviderAccountAction = "none" | "wait" | "relogin" | "inspect";
export type ProviderAccountDiagnosticSource = "cached" | "health" | "live_probe";
export type ProviderAccountProbeMode = "cached" | "health" | "live_probe";
export type ProviderAccountInventoryItem<Provider extends ProviderAccountProviderId = ProviderAccountProviderId> = {
...
export interface ProviderAccountRegistryPort<Account extends ProviderAccountInventoryItem = ProviderAccountInventoryItem> {
    listAccounts(input?: {
        readonly provider?: ProviderAccountProviderId;
    }): Promise<readonly Account[]>;
}
export interface ProviderAccountIdentityReaderPort<Account extends ProviderAccountInventoryItem = ProviderAccountInventoryItem> {
    readIdentity(input: {
        readonly account: Account;
        readonly now: Date;
    }): Promise<ProviderAccountIdentityReadResult>;
}
export interface ProviderAccountCapacityReaderPort<Account extends ProviderAccountInventoryItem = ProviderAccountInventoryItem> {
    readCapacity(input: {
        readonly account: Account;
        readonly identity: ProviderAccountIdentity;
        readonly now: Date;
...
export interface ProviderAccountHealthProbePort<Account extends ProviderAccountInventoryItem = ProviderAccountInventoryItem> {
    probeAccount(input: {
        readonly account: Account;
        readonly identity: ProviderAccountIdentity;
...
export type ListProviderAccountDiagnosticsDependencies<Account extends ProviderAccountInventoryItem = ProviderAccountInventoryItem> = {
