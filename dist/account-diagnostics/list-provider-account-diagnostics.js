import { chooseDiagnosticSignal, isSchedulerEligible, recommendedActionForAvailability, } from "./policy.js";
import { sanitizeDiagnosticDetails } from "./details.js";
export class ListProviderAccountDiagnostics {
    ...
    async execute(options = {}) {
        ...
        const diagnostics = await mapWithConcurrency(accounts, maxConcurrency, async (account) => this.diagnoseAccount({
            account,
            checkedAt,
            probeMode,
            probeSignals,
        ...
        const filtered = options.only?.length
            ? withSharedCapacity.filter((diagnostic) => options.only?.includes(diagnostic.availability))
            : withSharedCapacity;
        return {
            checkedAt,
    ...
    async diagnoseAccount(input) {
        const identityResult = await this.dependencies.identityReader.readIdentity({
            account: input.account,
            now: input.checkedAt,
        });
        const signals = [];
        if (identityResult.signal)
            signals.push(identityResult.signal);
        ...
        if (input.probeMode !== "cached" && this.dependencies.healthProbe) {
            signals.push(await this.readProbeSignal({
                account: input.account,
                identity: identityResult.identity,
        ...
        return {
            ...
            reconnectRequired: selected.reconnectRequired ??
...
function probeCacheKey(account, identity) {
    return [
        account.provider,
...
function markSharedCapacity(diagnostics) {
    const byHash = new Map();
    for (const diagnostic of diagnostics) {
        if (!diagnostic.accountKeyHash)
            continue;
...
//# sourceMappingURL=list-provider-account-diagnostics.js.map