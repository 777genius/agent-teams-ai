...
- Packet revision: `phase-01-pr252-base-conflict-resolution-r2`
- Durable controller: `controller-v17`; it must remain the same identity and exactly `live=true`
- Producer base, phase start, and expected integration target:
...
state, or authority. This r2 packet is its only authorized replacement. Never inspect, resume, or
reuse r1.
...
`p1-1d-shadowed-map-r4-accepted-integration-v3`, and accepted/pushed commit is the producer base above.
Those facts are immutable inputs, not work to rerun or reinterpret.

## Mission

Create one immutable resolution patch against the accepted target commit. The patch changes exactly
the five conflict paths below and resolves every one byte-for-byte to the blob from the pinned PR
source commit. It adds no authored product behavior and performs no merge, commit, push, integration,
or staging operation.

The producer may not choose a conflict side, combine hunks, reformat, regenerate, or make a semantic
edit. A missing source object, blob mismatch, unexpected conflict, required-check failure, or extra
path fails closed and returns to `controller-v17`.
...
All mandatory documents and both commits are read-only. The runtime/controller must materialize the
pinned source commit. The producer must not fetch, advance the branch, or substitute the current
remote-tracking ref when the pinned object is unavailable.
...
and `executionPolicy` is sandbox-only with network/fetch, app/runtime/team launch, real-project access,
staging, merge, commit, and push disabled. No empty collection is permission to discover a sibling.
...
pinned PR source commit. It does not mean the corresponding blob at expected target commit
`e7e7e734c82c49105682e7a19bbedafa1f5ddbad`.
...
1. prove `HEAD`, base, phase start, plan bundle, and expected target are exactly
   `e7e7e734c82c49105682e7a19bbedafa1f5ddbad` before the first edit;
2. prove the pinned source commit and all five required source blob OIDs are available without a
   fetch or moving-ref substitution;
3. replace each owned path with the complete bytes of its corresponding pinned source blob;
4. prove the worktree diff contains exactly all five owned paths, with no sixth path and nothing
   staged;
5. rerun every required check and full-blob verification below; and
6. return a runtime-owned immutable output for independent review, with explicit P0/P1/P2 counts and
   `nextAction: "integration-review"`.

The immutable output must bind the target commit, source remote/branch/commit, ordered five-path
scope, five expected and observed blob OIDs, exact diff, check results, typecheck-baseline
classification, safety classifications, and proof that producer merge/commit/push/stage operations
were not performed. Missing, ambiguous, mutable, or partial output is not reviewable.
...
The reviewer independently materializes the immutable producer output against the exact target,
reruns every check, verifies the exact five-path conflict set and full-blob mapping, and returns one
explicit `ACCEPT` or `REJECT` with complete P0/P1/P2 findings. `ACCEPT` is legal only with P0/P1/P2
`0/0/0` and complete green evidence. Blocked, incomplete, ambiguous, or missing output is not
acceptance.
...
overridden source/target fields. The runtime resolves the bound record, recreates the merge from the
two pinned commits, proves that the complete conflict set is exactly the five owned paths, applies
only the reviewed resolution bytes, reruns the required checks, and creates the true two-parent merge.

The final merge must have parents, in order,
`[e7e7e734c82c49105682e7a19bbedafa1f5ddbad,
7afc908ce92f14b4b0ebd06cc4aa3a4cf33807d0]`. Its tree must resolve the five paths to the five required
blob OIDs above, and no non-conflict path may differ from the true merge result. A synthetic one-parent
commit, patch-only commit, squashed commit, reversed parent order, moving branch head, missing/extra
conflict, or blob mismatch fails integration.
...
Stop and return to `controller-v17` on a stale/mixed commit, unavailable pinned source, source blob
mismatch, non-live/replaced controller, r1 access, second producer/reviewer, Fast mode, non-default
service tier, non-xhigh reasoning, extra/missing conflict or changed path, staged file, unsafe value,
binary, required-check drift/failure, producer/reviewer merge or Git write, or an integration attempt
that accepts anything other than the reviewed output ID.
