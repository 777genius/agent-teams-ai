...
Only [P3.A product binding](lanes/p3-a-product-binding.md) is launchable. It first proves whether the
existing PR #252 composition already satisfies the boundary. It may add code only for a demonstrated
missing seam and only inside its declared paths. Zero code is preferable when the current signed-v4
composition is already sufficient.
