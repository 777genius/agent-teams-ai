...
int main(int argc, char **argv) {
  ...
  printf("type=ready protocolVersion=1 spawnNonceHash=%s purpose=%s resetGeneration=%lu "
         "deploymentGeneration=%s processAnchorGeneration=%s anchorIdentity=anchor-%ld "
         "mainPidfdReady=true ownedProcessGroupReady=true\n",
         nonce, purpose, reset_generation, deployment_generation, process_anchor_generation,
         (long)getpid());
  fflush(stdout);
  ...
  if (residual_count != 0 || saw_escape || saw_ambiguous) {
    ...
    printf("type=drain_diagnostic residual=%zu escapedGroup=%s ambiguousIdentity=%s "
           "numericPidSignal=false numericProcessGroupSignal=false\n",
           residual_count, saw_escape ? "true" : "false", saw_ambiguous ? "true" : "false");
    printf("type=unclassified_residual protocolVersion=1 kind=process_drain_outcome_v1 outcome=unclassified "
           ...
           "reason=unclassified_identity containerReplacementRequired=true\n",
           purpose, reset_generation, deployment_generation, process_anchor_generation,
           process_anchor_generation, (long)getpid(), residual_labels);
    fflush(stdout);
    return residual_count == 0 ? 0 : 2;
  }
  printf("type=drain_diagnostic residual=0 pidfd=true subreaper=true numericPidSignal=false "
         "numericProcessGroupSignal=false\n");
  printf("type=drained protocolVersion=1 kind=process_drain_outcome_v1 outcome=drained purpose=%s "
         ...
         "classificationId=anchor-%s-%ld residuals=[]\n",
         purpose, reset_generation, deployment_generation, process_anchor_generation,
