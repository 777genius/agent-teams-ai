...
def run_anchor_case(
    ...
    nonce = f"spawn-nonce-hash-{mode}-{'eof' if close_control else 'stop'}"
    ...
    parse_ready_dto(ready, nonce, f"process-anchor-generation-{mode}")
...
def parse_ready_dto(
    ready: str, spawn_nonce_hash: str, process_anchor_generation: str
) -> dict[str, object]:
    fields = dict(part.split("=", 1) for part in ready.split() if "=" in part)
    expected_fields = {
        "type",
        "protocolVersion",
        "spawnNonceHash",
    ...
    check(set(fields) == expected_fields, "ready DTO fields differ from the protocol")
    dto: dict[str, object] = {
        "protocolVersion": int(fields.get("protocolVersion", "-1")),
        "spawnNonceHash": fields.get("spawnNonceHash"),
        ...
        "anchorIdentity": fields.get("anchorIdentity"),
        "mainPidfdReady": fields.get("mainPidfdReady") == "true",
        "ownedProcessGroupReady": fields.get("ownedProcessGroupReady") == "true",
    }
    check(dto["protocolVersion"] == 1, "ready DTO protocol mismatch")
    check(dto["spawnNonceHash"] == spawn_nonce_hash, "ready DTO nonce hash mismatch")
    check(dto["purpose"] == "host_reset", "ready DTO purpose mismatch")
    check(dto["resetGeneration"] == 7, "ready DTO reset generation mismatch")
    check(
        dto["deploymentGeneration"] == "deployment-generation-fixture",
        "ready DTO deployment generation mismatch",
    )
    check(
        dto["processAnchorGeneration"] == process_anchor_generation,
        "ready DTO process-anchor generation mismatch",
    )
    check(bool(dto["anchorIdentity"]), "ready DTO anchor identity missing")
    check(dto["mainPidfdReady"] is True, "ready DTO pidfd readiness missing")
    check(dto["ownedProcessGroupReady"] is True, "ready DTO process-group readiness missing")
    return dto
...
def parse_drain_dto(output: str, record_type: str) -> dict[str, object]:
    ...
    if record_type == "unclassified_residual":
        expected_fields.update({"reason", "containerReplacementRequired"})
    check(set(fields) == expected_fields, f"{record_type} DTO fields differ from the protocol")
    ...
    if record_type == "unclassified_residual":
        dto["reason"] = fields.get("reason")
        dto["containerReplacementRequired"] = (
            fields.get("containerReplacementRequired") == "true"
        )
    check(dto["protocolVersion"] == 1, "drain DTO protocol mismatch")
...
def run_process_anchor(binary: Path, marker: Path, owned: OwnedResources) -> dict[str, object]:
    ...
    ready_dto = parse_ready_dto(
        normal_ready, "spawn-nonce-hash-normal-stop", "process-anchor-generation-normal"
    )
    del ignore_ready, double_ready, escape_ready
    check(
    ...
    check(
        ...
        and "numericPidSignal=false" in escaped
        and "numericProcessGroupSignal=false" in escaped
        and "containerReplacementRequired=true" in escaped,
