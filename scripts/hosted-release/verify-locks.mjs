...
export async function verifyHostedLocksAtRoot(root, options = {}) {
  ...
  if ((await inspectEntry(legacyPath)) !== null) {
    throw new Error(
  ...
  const [ownerBytes, stackBytes] = await Promise.all([
    readStableLockFile(ownerPath, entries[0], options.onFileOpened),
    readStableLockFile(stackPath, entries[1], options.onFileOpened),
  ]);
  await assertDirectoryAncestryUnchanged(ancestry);
...
async function readStableLockFile(filePath, entryBefore, onFileOpened) {
  if (
  ...
  try {
    ...
    if (onFileOpened) await onFileOpened(path.basename(filePath));
