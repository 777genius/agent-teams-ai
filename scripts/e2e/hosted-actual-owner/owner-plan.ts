...
export function selectOwnerPlan(admission: PreflightAdmission) {
  ...
  if (!launch) return { protocol: legacyOwnerChildPlan(), image: admission.execution.ownerEntry.pin,
    argv: legacyOwnerChildPlan().wrapperArgv, selection: {} };
  const { selection: s } = launch;
  check(s.protocolVersion === 2 && s.recipeSha256 === admission.descriptor.p3b2.recipeSha256 &&
    s.recipeSha256 === admission.descriptor.p3b2.recipe.sha256 &&
    canonicalJson(launch.executable.pin) === canonicalJson(s.executable) &&
    canonicalJson(launch.helper.pin) === canonicalJson(s.helper), 'admitted_images');
  check(canonicalJson(admission.execution.ownerEntry.pin) === canonicalJson(admission.descriptor.p3b2.entry) &&
    ...
    s.source.executable.device === s.executable.device && s.source.executable.inode === s.executable.inode &&
...
export function assertOwnerPlanV2(plan: SupervisorPlan): void {
  ...
  check(helper && helper.root === 'p3b2' && helper.mode === 0o500 && helper.nlink === 1 &&
    ...
    /^[0-9a-f]{64}$/u.test(plan.ownerHarnessContractSha256 ?? ''), 'selected_helper_recipe');
  const manifest = plan.runtimeManifest;
  check(manifest.runId === plan.runId && manifest.refs.openCodeExecutableSha256 === plan.expectedExecutableSha256.opencode &&
    manifest.schemaVersion === 1 && manifest.purpose === 'agent-teams.hosted-actual-owner-e2e/v1', 'manifest');
  if (s) {
    check(s.format === 'agent-teams.hosted-owner-source-invocation/v1' &&
      s.executable.device === plan.expectedExecutableDevice.owner && s.executable.inode === plan.expectedExecutableInode.owner &&
      s.executable.sha256 === plan.expectedExecutableSha256.owner && s.module.sha256 === plan.expectedProducerModuleSha256.owner &&
      typeof s.module.path === 'string' && s.module.path.startsWith('/p3b2/') &&
      !s.module.path.includes('\\') && s.module.path.split('/').slice(1).every(p => p && p !== '.' && p !== '..') &&
      Buffer.byteLength(s.module.path) <= 4096 && !s.module.path.includes('\0') &&
      canonicalJson(plan.expectedArgv.owner) === canonicalJson(['run', s.module.path, ...OWNER_V2_ARGV]), 'source_selection');
  } else {
    check(plan.expectedExecutableSha256.owner === plan.expectedProducerModuleSha256.owner &&
      canonicalJson(plan.expectedArgv.owner) === canonicalJson(OWNER_V2_ARGV), 'built_selection');
  }
}
...
export function assertSelectedSupervisorTranscript(
  bytes: Uint8Array, plan: SupervisorPlan, supervisor: { pid: number; startTime: string },
  receipt?: SupervisorTranscriptReceipt
): void {
  assertOwnerPlanV2(plan);
  check(receipt && receipt.pid === supervisor.pid && receipt.startTime === supervisor.startTime &&
    receipt.transcriptSha256 === sha256(bytes), 'selected_transcript_receipt');
}
