...
function parseClosureManifest(bytes: Buffer, privateImagePaths: ReadonlySet<string>): readonly ClosureEntry[] {
  let value: unknown;
  try {
  ...
  const entries = value.map((candidate) => {
    ...
    if (
      !(privateImagePaths.has(path) ? item.mode === 0o500 : [0o444, 0o555].includes(item.mode as number)) ||
...
async function walkDirectory(
  ...
  manifestPath: string,
  output: ClosureEntry[],
  privateImagePaths: ReadonlySet<string>
): Promise<void> {
  ...
  for (const name of names) {
    ...
    if (childDirectory) {
      try {
        ...
        if (
          ...
          throw new Error('p3c_closure_directory_metadata');
        await walkDirectory(root, childDirectory, path, manifestPath, output, privateImagePaths);
      } finally {
    ...
    try {
      ...
      if (
        !before.isFile() ||
        before.nlink !== 1n ||
        !(privateImagePaths.has(path) ? mode === 0o500 : [0o444, 0o555].includes(mode)) ||
        before.size < 1n ||
...
export async function verifyClosure(
  ...
  // Only the exact v2 recipe-selected images may be private executable files. Legacy walks are unchanged.
  if (privateImages.some(p => root.name !== 'p3b2' || p.root !== root.name || p.mode !== 0o500))
    throw new Error('p3c_closure_private_image_selection');
  const privateImagePaths = new Set(privateImages.map(p => p.relativePath));
  if (privateImagePaths.size !== privateImages.length) throw new Error('p3c_closure_private_image_alias');
  ...
  const declared = parseClosureManifest(manifestBytes, privateImagePaths);
