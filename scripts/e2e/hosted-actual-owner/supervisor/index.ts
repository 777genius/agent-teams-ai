...
import {
  nativeActivationSocketIdentity,
  type NativeActivationSocketIdentity,
} from '../../../../src/main/composition/hosted/hostedNativeActivationSocketIdentity';
...
import {
  ...
  type SelectedSupervisorProcessObservation,
...
import {
  assertSelectedLaunchPreflightBound,
  encodeSelectedLaunchPhase,
  writeSelectedLaunchPhase,
  SELECTED_LAUNCH_PHASE,
} from './selected-launch-phase';
...
import {
  observeOwnerWrapper,
  observeSelectedOwnerModule,
  type OwnerWrapperObservation,
} from '../owner-wrapper-observation';
import { OWNER_LAUNCH_EVIDENCE_V2, type OwnerLaunchEvidenceV2 } from '../owner-descriptor-v2';
import { canonicalJson, sha256 } from './canonical';
import {
  assembleOwnerBootstrap,
  type AssembleBootstrapInput,
  type AssembledBootstrap,
} from './bootstrap-v2';
import {
  launchNativeOwner,
  type NativeLaunchOptions,
  type NativeOwnerLaunch,
} from './native-launch';
import { descriptorMap, OWNER_ROLES, wrapperStartToken, type HeldOwner } from './native-protocol';
import {
  SERVER_AUTH_V2,
  validatePreparedProfileTransfer,
  type RetainedPreparedProfile,
} from './private-profile-transfer';
...
export interface LaunchOwnerFromPlanOptions {
  ...
  readonly handles: Pick<
    NativeLaunchOptions,
    'helper' | 'executable' | 'cwdFd' | 'rawFd' | 'walFd'
  >;
  ...
  readonly environment: Omit<
    NativeLaunchOptions['environment'],
    'CLAUDE_TEAM_PRODUCER_PROVENANCE_V2'
  >;
  ...
  readonly activationHandleIpc?: Readonly<{
    ...
    readonly authorizeSuccessorHandle?: (
      selection: NativeActivationHandleSelection,
      endpoint: NativeActivationSocketIdentity
    ) => Promise<NativeSuccessorHandle>;
...
async function observeExecutingSupervisor(
  plan: SupervisorPlan,
  signal?: AbortSignal
): Promise<void> {
  ...
  try {
    ...
    check(
      stat.isFile() &&
        ...
        String(stat.ino) === plan.expectedExecutableInode.supervisor,
      'supervisor_image_identity'
    );
    const hash = createHash('sha256');
    let count = 0;
    for await (const b of createReadStream('', {
      fd,
      autoClose: false,
      start: 0,
      ...
      count += (b as Buffer).length;
    }
    const after = fstatSync(fd, { bigint: true });
    check(
      count === Number(stat.size) &&
        after.size === stat.size &&
        after.mtimeNs === stat.mtimeNs &&
        after.ctimeNs === stat.ctimeNs &&
        hash.digest('hex') === plan.expectedExecutableSha256.supervisor,
...
function parentCleanup(held: HeldOwner, ownerProcessStartToken: string) {
  return Object.freeze({
    ...
    wrapperPid: held.parentPid,
    wrapperStartTicks: held.parentStartTicks,
    wrapperStartToken: wrapperStartToken(held),
    ownerPid: held.ownerPid,
    ownerStartTicks: held.ownerStartTicks,
    ownerProcessStartToken,
    spawnNonce: held.spawnNonce,
    spawnBoundaryMonotonicNs: held.forkMonotonicNs,
    descriptors: Object.freeze(
      held.descriptors.map((d, i) =>
        Object.freeze({
          role: OWNER_ROLES[i][0],
          parentFd: d.sourceFd,
          beforeSpawn: Object.freeze({
            method: 'proc-fd-identity' as const,
            observedMonotonicNs: held.beforeMonotonicNs,
            path: `/proc/${held.parentPid}/fd/${d.sourceFd}`,
            device: d.device,
            inode: d.inode,
            mode: d.mode,
          }),
          afterSpawn: Object.freeze({
            ...
            observedMonotonicNs: d.closedMonotonicNs,
...
export async function launchOwnerFromPlan(options: LaunchOwnerFromPlanOptions) {
  ...
  check(
    plan.schemaVersion === 2 && plan.protocol === 'agent-teams.p3c.supervisor-transcript/v1',
    'version'
  );
  check(
    plan.controllerNonce === bootstrap.expectedHost.activation.controllerNonce &&
      plan.runId === bootstrap.expectedHost.activation.runId &&
      plan.startSchedule.some(
        (s) => s.role === 'owner' && s.generation === bootstrap.common.ownerGeneration
      ),
    'launch_binding'
  );
  for (const value of [
    options.supervisorProcessStartToken,
    options.recipeSha256,
    options.harnessContractSha256,
  ]) {
  ...
  check(
    cwd.isDirectory() &&
      String(cwd.dev) === plan.expectedCwd.owner.device &&
      String(cwd.ino) === plan.expectedCwd.owner.inode,
    'cwd'
  );
  check(
    bootstrap.expectedHost.executable.sha256 === plan.expectedExecutableSha256.opencode &&
      ...
      bootstrap.expectedHost.executable.moduleSha256 ===
        plan.expectedProducerModuleSha256.opencode &&
      bootstrap.expectedHost.executable.artifactManifestSha256 ===
        plan.expectedProducerArtifactSha256.opencode,
    'opencode_pins'
  );
  check(
    handles.executable.pin.sha256 === plan.expectedExecutableSha256.owner &&
      handles.executable.pin.device === plan.expectedExecutableDevice.owner &&
      handles.executable.pin.inode === plan.expectedExecutableInode.owner,
    'selected_owner_image'
  );
  const entrySha256 = plan.expectedProducerModuleSha256.owner;
  if (invocation.kind === 'built-entry') {
    check(
      plan.ownerSourceInvocation === undefined && handles.executable.pin.sha256 === entrySha256,
      'built_owner_image'
    );
  } else {
    const selected = plan.ownerSourceInvocation;
    check(
      selected?.format === 'agent-teams.hosted-owner-source-invocation/v1' &&
        ...
        selected.module.path === invocation.modulePath &&
        selected.module.sha256 === entrySha256,
      'source_selection'
    );
    check(
      invocation.moduleSha256 === entrySha256 &&
        invocation.modulePath.startsWith('/') &&
        Buffer.byteLength(invocation.modulePath) <= 4096 &&
        !invocation.modulePath.includes('\0') &&
        !invocation.modulePath.split('/').some((s) => s === '.' || s === '..'),
      'source_module'
    );
  }
  assertOwnerPlanV2(plan);
  check(
    plan.ownerPreparationModule
      ? bootstrap.serverAuthFormat === SERVER_AUTH_V2
      : bootstrap.serverAuthFormat !== SERVER_AUTH_V2,
    'profile_recipe_selection'
  );
  if (plan.supervisorSourceInvocation) {
    ...
    check(
      options.selectedPlanAdmission.process === options.selectedSupervisorObservation,
      'selected_supervisor_admission_observation'
    );
    assertSelectedSupervisorObservation(
      options.selectedSupervisorObservation,
      plan.supervisorSourceInvocation
    );
    check(
      options.selectedSupervisorObservation.processStartToken ===
        options.supervisorProcessStartToken &&
        options.selectedSupervisorObservation.executable.sha256 ===
          plan.expectedExecutableSha256.supervisor &&
        options.selectedSupervisorObservation.executable.device ===
          plan.expectedExecutableDevice.supervisor &&
        options.selectedSupervisorObservation.executable.inode ===
          plan.expectedExecutableInode.supervisor,
  ...
  if (options.activationHandleIpc) {
    check(
      options.activationHandleIpc.nativeController &&
        options.activationHandleIpc.nativeController.admission === options.selectedPlanAdmission &&
        canonicalJson(options.activationHandleIpc.nativeController.plan) === canonicalJson(plan),
      'native_controller_binding'
    );
    if (invocation.kind === 'built-entry') {
      // Mount metadata belongs to the authenticated recipe FilePin, not the
      // inherited FD's narrower image pin. Keep full descriptor equality here
      // and compare every inherited image field at the native launch boundary.
      const admitted = options.selectedPlanAdmission;
      check(admitted, 'compiled_plan_admission_required');
      assertSelectedPlanAdmission(admitted, plan);
      const image = admitted.ownerExecutable,
        inherited = handles.executable.pin;
      check(
        image.root === 'p3b2' &&
          image.relativePath === 'bin/hosted-owner' &&
          image.mode === 0o500 &&
          image.nlink === 1 &&
          plan.supervisorAdmissionDescriptor &&
          canonicalJson(image) === canonicalJson(plan.supervisorAdmissionDescriptor.p3b2.entry) &&
          inherited.device === image.device &&
          inherited.inode === image.inode &&
          inherited.size === image.size &&
          inherited.mode === image.mode &&
          inherited.sha256 === image.sha256,
        'compiled_mount_image'
      );
    }
    const artifacts = options.activationHandleIpc.selectedArtifacts;
    check(
      artifacts?.contract === SELECTED_LAUNCH_PHASE &&
        /^[0-9a-f]{64}$/u.test(artifacts.runtimeManifestSha256) &&
        artifacts.harnessContractPath.startsWith('/') &&
        artifacts.harnessContractPath.length <= 4096 &&
        !artifacts.harnessContractPath.includes('\0') &&
        !artifacts.harnessContractPath.includes('\\') &&
        artifacts.harnessContractPath
          .split('/')
          .slice(1)
          .every((p) => p && p !== '.' && p !== '..'),
      'selection_phase_artifacts'
    );
    check(
      options.activationHandleIpc.contract === 'agent-teams.hosted-native-activation-transfer/v1' &&
        plan.productSourceInvocation?.format === 'agent-teams.hosted-product-node-handle-ipc/v1' &&
        canonicalJson(plan.expectedArgv.product) ===
          canonicalJson([
            plan.productSourceInvocation.module.path,
            plan.productSourceInvocation.activationArgument,
          ]),
      'handle_transfer_version'
    );
  }
  check(
    options.recipeSha256 === plan.ownerRecipeSha256 &&
      options.harnessContractSha256 === plan.ownerHarnessContractSha256,
    'selected_contract_recipe'
  );
  const helper = plan.ownerLaunchHelper!;
  check(
    handles.helper.pin.device === helper.device &&
      handles.helper.pin.inode === helper.inode &&
      handles.helper.pin.sha256 === helper.sha256 &&
      handles.helper.pin.size === helper.size &&
      handles.helper.pin.mode === helper.mode,
    'selected_helper'
  );
  if (invocation.kind === 'source-bun')
    await observeSelectedOwnerModule(invocation.modulePath, invocation.moduleSha256);
  ...
  check(
    provenance.contract === 'claude-team/hosted-producer-provenance' &&
      provenance.version === 2 &&
      provenance.environment === 'CLAUDE_TEAM_PRODUCER_PROVENANCE_V2' &&
      provenance.descriptorSlots.ownerWalTimeline === 9 &&
      /^[0-9a-f]{64}$/u.test(provenance.contractSha256),
    'selected_provenance_contract'
  );
  ...
  const capsule = canonicalJson({
    activation: {
      ...
      runId: plan.runId,
      stackManifestSha256: bootstrap.expectedHost.activation.stackManifestSha256,
    },
    contract: provenance.contract,
    contractSha256: provenance.contractSha256,
    expectedProducer: {
      artifactManifestSha256: plan.expectedProducerArtifactSha256.owner,
      executableSha256: handles.executable.pin.sha256,
      implementationId: 'agent-teams.orchestrator.hosted-approval-owner.v1',
      moduleSha256: entrySha256,
    },
    producerRole: 'owner',
    streams: { ownerWalTimeline: { fd: 9, device: String(wal.dev), inode: String(wal.ino) } },
    version: 2,
  });
  ...
  check(
    bootstrap.serverAuthFormat === SERVER_AUTH_V2
      ? !!options.retainedPreparedProfile
      : !options.retainedPreparedProfile,
    'retained_profile_selection'
  );
  ...
  const profileContext = {
    expectedHostSha256: sha256(canonicalJson(bootstrap.expectedHost)),
    ownerProcessStartToken,
    bootstrapDigest: bootstrap.common.bootstrapBinding.bootstrapDigest,
  };
  let preparedProfileTransfer = options.retainedPreparedProfile
    ? structuredClone(options.retainedPreparedProfile.transfer(bootstrap.key, profileContext))
    : undefined;
  const preparedProfileBinding =
    preparedProfileTransfer &&
  ...
  const selectedStatic = options.activationHandleIpc
    ? Object.freeze(
        structuredClone({
          ...(preparedProfileBinding ? { preparedProfileBinding } : {}),
          expectedHost: bootstrap.expectedHost,
          rawRetention: bootstrap.rawRetention,
          supervisorProcessStartToken: options.supervisorProcessStartToken,
          ownerProcessStartToken,
          recipeSha256: options.recipeSha256,
          harnessContractSha256: options.harnessContractSha256,
          ownerModule: {
            path: plan.ownerSourceInvocation?.module.path ?? '/p3b2/bin/hosted-owner',
            sha256: entrySha256,
          },
          ownerArtifactManifestSha256: plan.expectedProducerArtifactSha256.owner,
          ownerExecutable: {
            device: handles.executable.pin.device,
            inode: handles.executable.pin.inode,
            size: handles.executable.pin.size,
            sha256: handles.executable.pin.sha256,
          },
          supervisorExecutable: {
            device: plan.expectedExecutableDevice.supervisor,
            inode: plan.expectedExecutableInode.supervisor,
            sha256: plan.expectedExecutableSha256.supervisor,
          },
          ...(options.selectedSupervisorObservation
            ? {
            ...
            : {}),
          refs: {
            openCode: plan.runtimeManifest.refs.openCode,
            orchestrator: plan.runtimeManifest.refs.orchestrator,
            product: plan.runtimeManifest.refs.product,
          },
          serializedProductBootstrap: new TextDecoder('utf-8', { fatal: true }).decode(
            bootstrap.serializedProductBootstrap
          ),
          producerCapsule: capsule,
          runtimeManifestSha256:
            options.activationHandleIpc!.selectedArtifacts.runtimeManifestSha256,
  ...
  const launch = await launchNativeOwner({
    ...handles,
    ...
    environment: {
      ...options.environment,
      P3C_PROCESS_OWNERSHIP_MARKER: plan.processOwnership.marker,
      [provenance.environment]: capsule,
    },
    async assemble(held) {
      wrapperObservation = await observeOwnerWrapper(held, handles.helper.pin);
      const assembled = assembleOwnerBootstrap({
        ...bootstrap,
        preparedProfileTransfer,
        held,
        supervisorBinding: {
          supervisorPid: held.callerPid,
          supervisorStartTicks: held.callerStartTicks,
          supervisorStartToken: options.supervisorProcessStartToken,
          supervisorExecutableSha256: plan.expectedExecutableSha256.supervisor,
          recipeSha256: options.recipeSha256,
          ownerEntrySha256: entrySha256,
          ownerPid: held.ownerPid,
          ownerStartTicks: held.ownerStartTicks,
          ownerProcessStartToken,
          harnessContractSha256: options.harnessContractSha256,
      ...
      digests = assembled.digests;
      return assembled;
    },
  });
  const map = descriptorMap(launch.held),
    cleanup = parentCleanup(launch.held, ownerProcessStartToken);
  const launchEvidence = (): OwnerLaunchEvidenceV2 =>
    Object.freeze({
      schemaVersion: 2,
      contract: OWNER_LAUNCH_EVIDENCE_V2,
      ownerProcessStartToken,
      descriptorMap: map,
      descriptorMapSha256: digests!.descriptorMapSha256,
      bootstrapDigests: digests!,
      parentCleanup: cleanup,
      wrapperObservation: wrapperObservation!,
      executedImageSha256: launch.executed.executableSha256,
      nativeEvents: launch.nativeEvents(),
    });
  let activationHandleTransfer:
    | Awaited<ReturnType<typeof transferNativeActivationHandle>>
    | undefined;
  ...
  if (options.activationHandleIpc) {
    ...
    try {
      ...
      const nativeAdmission = await options.activationHandleIpc.nativeController.nativeAdmission(
        {
          kind: 'native',
          generation: bootstrap.common.ownerGeneration,
          ownerProcessStartToken,
          launch: selectedLaunch,
          sealed: launch.sealed,
        },
        admissionSignal
      );
      const frame = encodeSelectedLaunchPhase(selectedLaunch, launch.sealed, nativeAdmission);
      try {
        selectedLaunchPhase = await writeSelectedLaunchPhase(
          endpoint,
          frame,
          deadline,
          admissionSignal
      ...
      const selected = decodeNativeActivationHandleSelection({
        contract: NATIVE_ACTIVATION_HANDLE_CONTRACT,
        ownerProcessStartToken,
        bootstrapV2HeaderSha256: digests!.bootstrapV2HeaderSha256,
        bootstrapDigest: digests!.bootstrapDigest,
        ownerGeneration: bootstrap.common.ownerGeneration,
        ownerSessionId: bootstrap.common.ownerSessionId,
        expectedOpenCodeExecutableSha256: bootstrap.expectedHost.executable.sha256,
      });
      const product =
        typeof options.activationHandleIpc.product === 'function'
          ? await options.activationHandleIpc.product(selected)
          : options.activationHandleIpc.product;
      const wire =
        selected.ownerGeneration === 1
          ? selected
          : await options.activationHandleIpc.authorizeSuccessorHandle?.(
              selected,
              nativeActivationSocketIdentity(endpoint)
            );
      ...
      activationHandleTransfer = await transferNativeActivationHandle(
        product,
        endpoint,
        wire,
        options.signal
      ...
      throw new OwnerPostExecHandoffError(
        error,
        Object.freeze({
          held: launch.held,
          sealed: launch.sealed,
