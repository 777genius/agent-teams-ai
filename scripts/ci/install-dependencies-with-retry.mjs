...
const RETRYABLE_NETWORK_ERROR =
  /(?:response code|status(?: code)?)[=: ]+5\d\d|\bERR_PNPM_FETCH_5\d\d\b|\b(?:EAI_AGAIN|ECONNRESET|ECONNREFUSED|ENETUNREACH|ETIMEDOUT)\b|socket hang up|network timeout/i;
  ...
  return new Promise((resolve, reject) => {
    ...
    for (const stream of [child.stdout, child.stderr]) {
      stream.on('data', (chunk) => {
        ...
        retryableFailureSeen ||= isRetryableInstallFailure(outputTail + text);
...
async function main() {
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    ...
    const canRetry = attempt < MAX_ATTEMPTS && result.retryableFailureSeen;
    if (!canRetry) {
