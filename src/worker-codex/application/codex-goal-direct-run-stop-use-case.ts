...
export async function stopDirectCodexGoalRun<TBrief extends DirectCodexGoalStopBrief>(input: {
  ...
  if (input.status.resultStatus === "done" || input.status.resultStatus === "completed") {
    return {
      ...
      safeMessage:
        "Direct-run job already has a terminal result. Review workspace/log/result before continuing or recovery.",
    };
  }
  if (input.brief.workerAlive) {
    return {
      ...
      safeMessage:
        "Direct-run worker appears alive without a tmux session. Use a provider-safe control signal or stop the owning process outside this tool, then reconcile.",
    };
  }
  ...
  return {
    ...
    safeMessage:
      "Direct-run job has no live worker and no tmux session. Reconciled runtime result; review workspace/log/result before continuing or recovery.",
  };
}
