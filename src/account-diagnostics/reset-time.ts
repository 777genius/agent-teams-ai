...
export function parseLimitResetFromText(input: {
  ...
  if (!rawResetText) return {};

  const relative = parseRelativeReset(rawResetText, input.now);
  if (relative) return { rawResetText, limitResetAt: relative };

  const absolute = parseAbsoluteReset(rawResetText, input.now);
  if (absolute) return { rawResetText, limitResetAt: absolute };
...
function extractResetText(text: string): string | null {
  ...
  if (explicitDate?.[1]) return explicitDate[1].trim();

  const clockTime = normalized.match(
    /\b(?:try again|reset(?:s)?|available|limit resets?)(?:[^.\n]{0,80})\bat\s+(\d{1,2}:\d{2}\s*(?:AM|PM))/i,
  );
  if (clockTime?.[1]) return clockTime[1].trim();

  const looseClockTime = normalized.match(/\b(\d{1,2}:\d{2}\s*(?:AM|PM))\b/i);
  return looseClockTime?.[1]?.trim() ?? null;
}

function parseRelativeReset(raw: string, now: Date): Date | null {
...
function parseAbsoluteReset(raw: string, now: Date): Date | null {
  ...
  if (clock) {
    ...
    const normalizedHours =
      meridiem === "AM" ? hours % 12 : hours === 12 ? 12 : hours + 12;
