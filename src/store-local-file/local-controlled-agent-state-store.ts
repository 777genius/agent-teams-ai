  ...
  implements ControllerStateStorePort, ControlledAgentEventPort
{
  ...
  private sessionDir(sessionId: string): string {
    return join(this.options.rootDir, "controlled-agent", "sessions", hashId(sessionId));
  }

  private runPath(runId: string): string {
    return join(this.options.rootDir, "controlled-agent", "runs", hashId(runId), "run.json");
  }
}
