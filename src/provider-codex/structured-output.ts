...
function parseLastJsonCandidate(value: string): JsonCandidate | null {
  const candidates = [...jsonCandidates(value)];
  let lastCandidate: JsonCandidate | null = null;
...
function outerJsonCandidates(
  candidates: readonly JsonCandidate[],
): readonly JsonCandidate[] {
  const outer = new Set<JsonCandidate>();
  let maxEnd = -1;
  for (const candidate of [...candidates].sort(
    (left, right) => left.index - right.index || right.end - left.end,
  )) {
    if (maxEnd >= candidate.end) continue;
    outer.add(candidate);
...
function* jsonCandidates(value: string): Generator<JsonCandidate> {
  yield* fencedJsonCandidates(value);
  yield* balancedJsonCandidates(value, nonJsonFenceRanges(value));
}

function* fencedJsonCandidates(value: string): Generator<JsonCandidate> {
...
): Generator<JsonCandidate> {
  const stack: JsonBracket[] = [];
  let inString = false;
  let escape = false;
...
function nonJsonFenceRanges(value: string): readonly TextRange[] {
...
function nextFence(
  value: string,
  searchFrom: number,
): { readonly start: number; readonly marker: string } | null {
  const backtick = value.indexOf("```", searchFrom);
  const tilde = value.indexOf("~~~", searchFrom);
  if (backtick < 0 && tilde < 0) return null;
...
function firstLineEnd(
  value: string,
  start: number,
): { readonly index: number; readonly end: number } {
  const lineFeed = value.indexOf("\n", start);
  if (lineFeed < 0) return { index: value.length, end: value.length };
  const carriageReturnIndex =
    lineFeed > start && value[lineFeed - 1] === "\r" ? lineFeed - 1 : lineFeed;
