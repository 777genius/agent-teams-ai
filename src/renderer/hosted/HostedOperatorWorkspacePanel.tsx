...
export const HostedOperatorWorkspacePanel = ({
  ...
  return (
    <section className="p-4" aria-label="Hosted operator workspace">
      {snapshot.refreshing ? <span className="sr-only" role="status">Refreshing hosted operator readiness…</span> : null}
      <HostedOperatorSurfaces
        readiness={snapshot.readiness}
        approvalDecisionsReady={snapshot.readiness.dimensions?.some(
