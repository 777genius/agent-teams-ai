...
export interface HostedDraftPublicationComposition {
  ...
  ): Promise<HostedDraftWorkspaceFence>;
  publishPromotionPlan(
...
export async function createHostedDraftPublicationComposition(input: {
  ...
  let directories: HostedDraftDirectoryPublisher | null = null;
  try {
    directories = await HostedDraftDirectoryPublisher.admit(
      bootstrap.runtimeInstance.claudeRoot.reference
    );
    publisher = createHostedDraftPublicationFeature({
      directories,
      identities: backend.gateway,
  ...
  return Object.freeze({
    journal: drafts.draftPublications,
    identities: backend.gateway,
    publisher,
    publishPromotionPlan: (
      operation: HostedPromotionRecord,
    ...
    captureWorkspace: async (
      workspaceId: WorkspaceId,
      authenticated: HostedAuthenticatedPrincipal,
      context: QueryContext,
      restoreGeneration: number
    ) => {
      ...
      const assertContext = () => {
        if (
          ...
          context.deploymentId !== bootstrap.deploymentId ||
          context.bootId !== bootstrap.bootId
        ) {
      ...
      const readGrant = async () => {
        assertContext();
        const rows = await drafts.gateway.hostedAuthCall('workspace.grant.list', {
          userId,
          grantGeneration: restoreGeneration,
        });
        if (!Array.isArray(rows) || rows.length > 256)
          throw new Error('draft-workspace-grants-invalid');
        ...
        if (
          row.userId !== userId ||
          row.grantGeneration !== restoreGeneration ||
          typeof row.grantRevision !== 'string' ||
          !/^[a-f0-9]{64}$/.test(row.grantRevision)
        ) {
          throw new Error('draft-workspace-grant-invalid');
        }
        const runtimeWorkspaceId = parseWorkspaceId(row.runtimeWorkspaceId);
        const registration =
          bootstrap.workspaceRegistrySnapshot.registry.requireEnabled(runtimeWorkspaceId);
        ...
        if (
          runtimeWorkspaceId !== bootstrap.workspaceId ||
          mount.workspaceId !== runtimeWorkspaceId ||
          mount.bootId !== context.bootId ||
          mount.health !== 'healthy' ||
          mount.declaredRootHash !== registration.declaredRootHash
        )
          throw new Error('draft-workspace-mount-invalid');
        assertContext();
        return {
          runtimeWorkspaceId,
          ...
          grantGeneration: restoreGeneration,
          mountGeneration: mount.mountGeneration,
          declaredRootHash: mount.declaredRootHash,
        };
      };
      ...
      return {
        ...
        bindingGeneration: 1,
        grantRevision: captured.revision,
        grantGeneration: captured.grantGeneration,
        assertCurrent: async () => {
          if (JSON.stringify(await readGrant()) !== JSON.stringify(captured))
            throw new Error('draft-workspace-fence-changed');
        },
      };
    },
    dispose: async () => {
...
/** Narrow host custody seam for exact frozen-plan publication and recovery. */
export async function createHostedPromotionPlanPublisher(claudeRoot: string): Promise<{
  publish(
    operation: HostedPromotionRecord,
    expectedFingerprint: string,
    assertCurrent: () => Promise<void>
  ...
  const directories = await HostedDraftDirectoryPublisher.admit(claudeRoot);
  return {
    publish: (operation, expectedFingerprint, assertCurrent) =>
      directories.publishPromotionPlan(operation, expectedFingerprint, assertCurrent),
    dispose: () => directories.dispose(),
  };
}
...
function fingerprint(directory: Directory): string {
  return createHash('sha256')
    .update(
      JSON.stringify({
        schemaVersion: 1,
        canonicalPath: directory.logicalPath,
        device: directory.stat.dev.toString(),
        inode: directory.stat.ino.toString(),
      }),
      'utf8'
    )
    .digest('hex');
...
async function current(directory: Directory): Promise<void> {
  const [named, retained, canonical] = await Promise.all([
    fs.lstat(directory.logicalPath, { bigint: true }),
  ...
  if (
    ...
    canonical !== directory.logicalPath ||
    !sameEntry(named, directory.stat) ||
    !sameEntry(retained, directory.stat) ||
    named.uid !== directory.stat.uid ||
    named.mode !== directory.stat.mode
  ) {
...
async function directory(
  ...
  if (!before.isDirectory() || before.isSymbolicLink())
    throw new Error('draft-publication-unsafe-directory');
...
async function readExact(
  ...
  try {
    ...
    if (
      ...
      (exactMode && (before.mode & 0o777n) !== 0o600n) ||
    ...
    if (
      offset !== bytes.length ||
      !observed.subarray(0, offset).equals(bytes) ||
      !sameEntry(before, named) ||
...
class HostedDraftDirectoryPublisher implements HostedDraftDirectoryPublicationPort {
  ...
  static async admit(claudeRoot: string): Promise<HostedDraftDirectoryPublisher> {
    if (
      ...
      path.resolve(claudeRoot) !== claudeRoot ||
      claudeRoot === '/'
    ) {
  ...
  async withDirectory<T>(
    ...
    if (
      !/^adoption_[a-f0-9]{32}$/.test(request.operationId) ||
  ...
  /** The Owner reads this exact immutable file; a conflicting or partial file is never replaced. */
  async publishPromotionPlan(
    operation: HostedPromotionRecord,
    expectedFingerprint: string,
    assertCurrent: () => Promise<void>
  ): Promise<void> {
    const bytes = Buffer.from(operation.planJson, 'utf8');
    if (
      createHash('sha256').update(bytes).digest('hex') !== operation.planSha256 ||
      operation.planGeneration !== `plan-generation_${operation.planSha256}` ||
    ...
    const publication = await this.withDirectory(
      ...
      async (lease) => {
        const key = parseLegacyTeamKey(`draft-${operation.createOperationId.slice(9)}`);
        const team = await directory(
          child(this.teams, key),
          path.join(this.teams.logicalPath, key),
          true
        );
        try {
          if (fingerprint(team) !== lease.fingerprint)
            throw new Error('promotion-directory-mismatch');
          const assertEffect = async () => {
