...
import { hasTrustworthyDurablePathIdentity, isSameDurablePathIdentity } from './durablePathIdentity';
...
export function isDurablePathIdentity(value: unknown): value is DurablePathIdentity {
  ...
  return Number.isSafeInteger(identity.dev) && Number.isSafeInteger(identity.ino) &&
    Number.isFinite(identity.birthtimeMs) && hasTrustworthyDurablePathIdentity(identity);
}

export function isDurableFileIdentity(value: unknown): value is DurableFileIdentity {
  return isDurablePathIdentity(value) && Number.isFinite((value as DurableFileIdentity).size) &&
...
export function parseAtomicCreateRecoveryRecord(
  ...
  try {
    ...
    if (
      record.version !== 1 || !isUuid(record.nonce) || !isUuid(record.cleanupAuthority) ||
      ...
      !isDurablePathIdentity(record.directoryIdentity) || !record.attachment ||
      !(
...
export function parseAtomicCreateDirectoryRetirementJournal(
  ...
  try {
    ...
    return journal.version === 1 && isDurablePathIdentity(journal.directoryIdentity) &&
