...
import { cleanupStockNodeAtomicCreateTempLinks } from './atomicWriteCleanup';
...
async function readBoundedRecoveryRecord(
  ...
  budget: AtomicCreateRecoveryBudget = createAtomicCreateRecoveryBudget()
  ...
  const beforeOpen = await withinAtomicCreateRecoveryBudget(budget, 'recovery-record-lstat', () =>
    lstatOrNull(recordPath)
  );
  ...
  const openedRecord = await readBoundedRegularTextWithIdentity(
    recordPath,
    MAX_RECOVERY_RECORD_BYTES,
    budget
  );
  const record = parseAtomicCreateRecoveryRecord(
    openedRecord.text,
    directoryName,
    allowRetiredDirectory
  );
  if (
    !record ||
  ...
  return { record, recordPath, identity: openedRecord.identity };
}
...
async function withValidatedRecoveryDirectory<T>(
  ...
  const initialStats = await withinAtomicCreateRecoveryBudget(
    budget,
    'recovery-directory-lstat',
    () => lstatOrNull(cleanupDirectory)
  );
  if (!initialStats || !initialStats.isDirectory() || initialStats.isSymbolicLink()) {
    throw new Error(
  ...
  const directoryHandle = await withinAtomicCreateRecoveryOpenBudget(
    ...
    () => fs.promises.open(cleanupDirectory, directoryOpenFlags())
  );
  let closeAfterPendingStat = false;
  try {
    const openedStats = await withinAtomicCreateRecoveryBudget(
      budget,
    ...
    if (!closeAfterPendingStat) await directoryHandle.close().catch(() => undefined);
  }
}
...
async function readRetirementJournal(
  ...
  for (const name of names) {
    ...
    if (
      await withinAtomicCreateRecoveryBudget(budget, 'retirement-journal-lstat', () =>
        lstatOrNull(pathname)
      )
    ) {
      if (journalPath)
        throw new Error(
  ...
  const beforeOpen = await withinAtomicCreateRecoveryBudget(
    budget,
    'retirement-journal-lstat',
  ...
  const journal = await readBoundedRegularTextWithIdentity(
    journalPath,
    MAX_RECOVERY_RECORD_BYTES,
    budget
  );
  if (!parseAtomicCreateDirectoryRetirementJournal(journal.text, directoryIdentity)) {
    throw new Error(`Atomic-create recovery directory journal changed: ${journalPath}`);
  }
  return { name: journalName, identity: journal.identity };
}
...
async function removeValidatedRecoveryRecordAndDirectory(
  ...
  await withValidatedRecoveryDirectory(
    ...
    async ({ stableDirectory, records, directoryIdentity }) => {
      if (records.length === 0) {
        const journal = await readRetirementJournal(stableDirectory, directoryIdentity, budget);
        if (
          journal &&
 ...
 */
async function recoverDetachedLinks(
  ...
  for (const entry of cleanupDirectories) {
    ...
    try {
        ...
        async ({ stableDirectory, records, directoryIdentity }) => {
            ...
            await Promise.all(
              records.map(async ({ record, recordPath }) => {
                ...
                return {
                  ...
                  record,
                  recordPath,
                  stats: await withinAtomicCreateRecoveryBudget(
                    budget,
                    'recovery-attachment-lstat',
                    () => lstatOrNull(pathname)
                  ),
                };
              })
          ...
          if (
            (
              await Promise.all(
                records.map(({ record }) =>
                  withinAtomicCreateRecoveryBudget(budget, 'recovery-attachment-lstat', () =>
      ...
      if (recovery?.recordless) {
        ...
        if (
          await withinAtomicCreateRecoveryBudget(budget, 'recovery-directory-lstat', () =>
            lstatOrNull(cleanupDirectory)
          )
        )
          retainedRecoveryAuthorities++;
...
export async function cleanupAtomicCreateTempLinks(targetPath: string): Promise<void> {
  const budget = createAtomicCreateRecoveryBudget();
  const publicDirectoryPath = path.dirname(targetPath);
  const targetName = path.basename(targetPath);
  // Stock Node has no exact-generation deletion; retain every crash prefix.
  if (!safeCleanupProtocolAvailable()) {
    await cleanupStockNodeAtomicCreateTempLinks(targetPath, budget);
    return;
  }
    ...
    async ({ stablePath: directoryPath, assertStillOwned }) =>
      withAtomicCreateCleanupCapacity(
        ...
        async () => {
          ...
          await recoverDetachedLinks(directoryPath, budget);

          const target = await withinAtomicCreateRecoveryBudget(budget, 'target-lstat', () =>
            lstatOrNull(path.join(directoryPath, targetName))
          );
          if (!target) return;
          ...
          for (const entry of entries) {
            ...
            try {
              const candidate = await withinAtomicCreateRecoveryBudget(
                budget,
                'public-guard-lstat',
              ...
              try {
                try {
                  await assertRecoveryDirectoryStillOwned(
                    cleanupDirectory,
                    record.directoryIdentity
                  );
                ...
                const detachedStats = detached
                  ? await withinAtomicCreateRecoveryBudget(
                      budget,
                      'detached-attachment-lstat',
                      () => lstatOrNull(detachedPath)
                    )
                  : null;
                if (
                  detached &&
...
function safeCleanupProtocolAvailable(): boolean {
  return false;
}
