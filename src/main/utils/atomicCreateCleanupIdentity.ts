...
export async function moveOwnedPrivateChild(
  ...
  if (!safeNamespaceMutationAvailable()) return false;
...
export async function claimOwnedPrivateDirectory(
  ...
  if (!safeNamespaceMutationAvailable()) return null;
  ...
  if (
    ...
    !isSameDurablePathIdentity(getDurablePathIdentity(actual), expectedIdentity)
  )
    return null;
...
function safeNamespaceMutationAvailable(): boolean {
  return false;
}
