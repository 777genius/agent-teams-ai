...
/**
 * The switch has to work on the path the app actually uses.
 *
 * The compatibility facade always supplies `isOpenCodePrimaryLaneSelfHealEnabled`,
 * so the tracker never falls back to its own default. While that port fell back
 * to the CONSTANT rather than the env reader, setting the environment variable
 * changed nothing in the running app - the switch existed only in tests that
 * constructed a tracker directly. This pins the wiring, not the reader.
 */
describe('the switch on the production wiring', () => {
  function trackerWithFacadeStyleFallback(env: NodeJS.ProcessEnv) {
    const rebootstrapPrimaryLane = vi.fn(async () => true);
    const tracker = new OpenCodePrimaryLaneBootstrapSelfHealTracker({
      nowMs: () => 1_000_000,
      inspectLaneStorage: async () => UNBOOTSTRAPPED_LANE_STORAGE,
      rebootstrapPrimaryLane,
      // Mirrors TeamProvisioningOpenCodeMemberMessageDeliveryCompatibilityFacade:
      // an optional dep, falling back to the env reader.
      isOpenCodePrimaryLaneSelfHealEnabled: () =>
        undefined ?? isOpenCodePrimaryLaneSelfHealEnabledFromEnv(env),
    });
    return { tracker, rebootstrapPrimaryLane };
  }
  ...
  it('stays off when the environment says nothing', async () => {
    const { tracker, rebootstrapPrimaryLane } = trackerWithFacadeStyleFallback({});
  ...
  it('turns on when the environment says so', async () => {
    const { tracker } = trackerWithFacadeStyleFallback({
      [OPENCODE_PRIMARY_LANE_SELF_HEAL_ENV]: '1',
    });

    // Enabled, so the ladder proceeds into its grace window instead of giving up.
    expect((await tracker.request(request)).action).toBe('wait');
  });
});
