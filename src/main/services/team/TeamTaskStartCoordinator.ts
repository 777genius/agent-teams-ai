...
type TaskStartBoard = Pick<
  AgentTeamsController['taskBoard'],
  'getTask' | 'listTasks' | 'listDeletedTasks' | 'createTask' | 'startTask'
>;
type TaskBoardWithCreationReconciliation = TaskStartBoard & {
  reconcileTaskCreation(input: Record<string, unknown>): unknown;
};

function hasTaskCreationReconciliation(
  taskBoard: TaskStartBoard
...
export interface TeamTaskStartCoordinatorPorts {
  getTaskBoard(teamName: string): TaskStartBoard;
