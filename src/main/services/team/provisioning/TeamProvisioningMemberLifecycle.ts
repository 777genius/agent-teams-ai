...
export interface TeamProvisioningMemberLifecycleHost {
  ...
  upsertRunAllEffectiveMember(run: ProvisioningRun, member: TeamCreateRequest['members'][number]): void;
  ...
  getLiveTeamAgentRuntimeMetadata(teamName: string): Promise<Map<string, LiveTeamAgentRuntimeMetadata>>;
  ...
  buildConfiguredProvisioningMember(member: EffectiveConfiguredMember): TeamCreateRequest['members'][number];
  ...
  launchSingleMixedSecondaryLane(run: ProvisioningRun, lane: MixedSecondaryRuntimeLaneState): Promise<void>;
  getMixedSecondaryLaunchPhase(run: ProvisioningRun): PersistedTeamLaunchPhase;
  writeLaunchStateSnapshot(teamName: string, snapshot: PersistedTeamLaunchSnapshot): Promise<unknown>;
...
export class TeamProvisioningMemberLifecycleController {
  ...
  private persistInboxMessage(teamName: string, memberName: string, message: Record<string, unknown>): void {
  ...
  private appendMemberBootstrapDiagnostic(run: ProvisioningRun, memberName: string, text: string): void {
  ...
  private upsertRunAllEffectiveMember(run: ProvisioningRun, member: TeamCreateRequest['members'][number]): void {
  ...
  private getLiveTeamAgentRuntimeMetadata(teamName: string): Promise<Map<string, LiveTeamAgentRuntimeMetadata>> {
  ...
  private resolveOpenCodeMemberWorkspacesForRuntime(
    input: Parameters<TeamProvisioningMemberLifecycleHost['resolveOpenCodeMemberWorkspacesForRuntime']>[0]
  ): Promise<TeamCreateRequest['members']> {
    return this.host.resolveOpenCodeMemberWorkspacesForRuntime(input);
  }

  private buildConfiguredProvisioningMember(member: EffectiveConfiguredMember): TeamCreateRequest['members'][number] {
  ...
  private launchSingleMixedSecondaryLane(run: ProvisioningRun, lane: MixedSecondaryRuntimeLaneState): Promise<void> {
  ...
  private writeLaunchStateSnapshot(teamName: string, snapshot: PersistedTeamLaunchSnapshot): Promise<unknown> {
