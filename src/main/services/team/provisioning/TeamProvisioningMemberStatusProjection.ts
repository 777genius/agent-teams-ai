...
export function resolveEffectiveConfiguredMember(
  ...
  const effort = isTeamEffortLevel(metaMember?.effort)
    ...
    : isTeamEffortLevel(configuredMember?.effort)
      ...
      : undefined;
  const fastMode =
    metaMember?.fastMode === 'inherit' || metaMember?.fastMode === 'on' || metaMember?.fastMode === 'off'
      ? metaMember.fastMode
      : configuredMember?.fastMode === 'inherit' ||
        ...
        : undefined;
  const agentType = metaMember?.agentType?.trim() || configuredMember?.agentType?.trim() || undefined;
  const mcpPolicy =
