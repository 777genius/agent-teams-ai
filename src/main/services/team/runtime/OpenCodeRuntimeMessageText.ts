      ...
      : isInformationalNotice
        ...
        : isTeammateReport
          ? [
              ...
              `If this report is the one that completes the work the user originally requested (this task was the last open task and the required deliverables now exist), send the user exactly one final message with teamName="${input.teamName}", to="user", from="${input.memberName}", source="runtime_delivery", text, and summary. If the board was already complete before this report, the final message was already sent in an earlier turn: do not send it again. A "Duplicate message ignored" result means the same.`,
