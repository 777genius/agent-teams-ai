...
export type HostedInitialLane =
  | Readonly<{ kind: 'native'; provider: Exclude<TeamProviderId, 'opencode'>; members: readonly HostedInitialMember[] }>
  | Readonly<{ kind: 'opencode'; provider: 'opencode'; selectedModel: string; effort?: EffortLevel; members: readonly HostedInitialMember[] }>;
...
function fields(value: Record<string, unknown>, required: readonly string[], optional: readonly string[] = []): void {
  if (required.some((key) => !Object.hasOwn(value, key)) ||
      Reflect.ownKeys(value).some((key) => typeof key !== 'string' || ![...required, ...optional].includes(key))) invalid();
}
...
function bounded(value: unknown, bytes: number): string {
  if (typeof value !== 'string' || !value.length || value.trim() !== value || value.includes('\0') ||
      value.length > bytes || new TextEncoder().encode(value).length > bytes) return invalid();
  return value;
}
...
export function isHostedInitialMemberName(name: string): boolean {
  ...
  return name.length <= 64 && name.trim() === name &&
    /^[A-Za-z0-9](?:[A-Za-z0-9._-]{0,62}[A-Za-z0-9])?$/.test(name) &&
    validateTeamMemberNameFormat(name) === null && lower !== 'user' && lower !== 'team-lead' &&
    !lower.endsWith('-provisioner') && !(suffix && suffix.suffix >= 2);
}
...
function model(value: unknown, provider: TeamProviderId): string {
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._:/[\]-]*$/.test(result) ||
      /^[A-Za-z]:/.test(result) || result.split('/').some((part) => part === '.' || part === '..')) return invalid();
...
export function parseHostedRosterConfiguration(value: unknown): HostedRosterConfiguration {
  ...
  if (input.schemaVersion !== 1 || (input.toolApprovalMode !== 'manual' && input.toolApprovalMode !== 'auto')) return invalid();
  ...
  const lanes = list(input.lanes).map((raw): HostedInitialLane => {
    ...
    fields(lane, ['kind', 'provider', 'members', ...(openCode ? ['selectedModel'] : [])], openCode ? ['effort'] : []);
    const members = Object.freeze(list(lane.members).map((rawMember): HostedInitialMember => {
      const member = record(rawMember);
      fields(member, ['name', 'prompt', ...(!openCode ? ['model'] : [])], openCode ? ['model', 'effort'] : ['effort']);
      ...
      return Object.freeze({ name, prompt: bounded(member.prompt, 64 * 1024),
    ...
    if (openCode) return Object.freeze({ kind: 'opencode', provider: 'opencode', selectedModel: model(lane.selectedModel, provider),
      ...(Object.hasOwn(lane, 'effort') ? { effort: effort(lane.effort, provider) } : {}), members });
    return Object.freeze({ kind: 'native', provider, members });
  });
  const result = Object.freeze({ schemaVersion: 1 as const, toolApprovalMode: input.toolApprovalMode, lanes: Object.freeze(lanes) });
  if (new TextEncoder().encode(JSON.stringify(result)).length > HOSTED_ROSTER_MAX_BYTES) return invalid();
  return result;
}

export function hostedRosterMembers(configuration: HostedRosterConfiguration): readonly Readonly<{ name: string }>[] {
  return Object.freeze(configuration.lanes.flatMap((lane) => lane.members.map(({ name }) => Object.freeze({ name }))));
}

export function assertHostedRosterMatches(configuration: HostedRosterConfiguration, members: readonly Readonly<{ name: string }>[]): void {
  const ordered = hostedRosterMembers(configuration);
  if (ordered.length !== members.length || ordered.some((member, index) => member.name !== members[index]?.name)) invalid();
}
