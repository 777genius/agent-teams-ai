...
Renderer message mutations are exposed through `renderer/index.ts`. The renderer
slice owns direct and cross-team send state, optimistic inbox projection, OpenCode
delivery diagnostics, and context-scoped cross-team target refreshes. The legacy
store remains the composition root and supplies transport, cache, analytics,
request-scope, and refresh ports.
