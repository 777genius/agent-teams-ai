...
export class MemberWorkSyncRecoveryCommands {
  ...
  async stop(input: {
    ...
    if (expectedRuntimeInstanceId || localStopId) {
      ...
      return this.stopExactRuntime({ ...input, expectedRuntimeInstanceId, localStopId });
    }
  ...
  private async stopExactRuntime(input: {
    ...
    const status = await runMemberWorkSyncStatusMutation(this.deps, async (mutationId) => {
      ...
      if (findMemberWorkSyncDurableStopReceipt(read.status.recoveryHealth, receiptScope)) {
        return read.status;
      }
      const nowIso = this.deps.clock.now().toISOString();
      const recoveryHealth = applyMemberWorkSyncStopLatch({
        previous: read.status.recoveryHealth,
        nowIso,
        reason: input.reason?.trim() || 'runtime_local_stop',
        durableReceipt: { ...receiptScope, appliedAt: nowIso },
      });
      const runtimeAdmission = await this.linearizeExactRuntimeStop({
        teamName: input.teamName,
        memberName: input.memberName,
        teamIncarnation: incarnation,
        runtimeInstanceId: input.expectedRuntimeInstanceId,
        controlRevision: recoveryHealth.controlRevision ?? 1,
      });
    ...
    return {
      ...
      status: nextStatus,
      code: 'stopped',
      runtimeAdmission: nextStatus.runtimeAdmission ?? { state: 'applied' },
    };
  }

  private async linearizeExactRuntimeStop(input: {
    teamName: string;
    memberName: string;
    ...
    const before = await admission.readLiveControl(input);
    if (before && before.runtimeInstanceId !== input.runtimeInstanceId) {
      throw new MemberWorkSyncStaleRuntimeInstanceError();
    }
    const result = await admission.syncControl({ ...input, stopped: true });
    if (result.ok) {
      return { state: 'applied', controlRevision: result.controlRevision };
    }
    if (result.code === 'instance_mismatch') {
      throw new MemberWorkSyncStaleRuntimeInstanceError();
    }
    const after = await admission.readLiveControl(input);
    if (after?.runtimeInstanceId !== input.runtimeInstanceId) {
      if (after) throw new MemberWorkSyncStaleRuntimeInstanceError();
      throw new MemberWorkSyncRuntimeControlUnavailableError();
    }
    if (after.stopped && after.controlRevision >= input.controlRevision) {
      return { state: 'applied', controlRevision: after.controlRevision };
    }
    throw new MemberWorkSyncRuntimeControlUnavailableError();
  }
