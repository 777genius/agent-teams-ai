...
export class HostedPromotionStorageOps {
  ...
  begin(value: unknown): HostedPromotionBeginResult {
    ...
    try {
      return db
        .transaction((): HostedPromotionBeginResult => {
          ...
          if (previous.length) {
            ...
            if (
              ...
              Object.entries(binding).some(
                ([key, field]) => operation[key as keyof HostedPromotionRecord] !== field
              )
            ) {
              return { kind: 'conflict', reason: 'operation_mismatch' };
            }
            const frozenDraft = parseHostedTeamConfigurationStorageDraft(
              JSON.parse(operation.frozenDraftJson)
            );
            if (
              frozenDraft.configuration &&
              !isHostedMvpApprovalModeAvailable(frozenDraft.configuration)
            ) {
          ...
          const saved = db
            .prepare(
              `SELECT members_json FROM hosted_team_configuration_drafts
          WHERE workspace_id = ? AND team_id = ?`
            )
            .get(workspaceId, teamId) as { members_json: string };
          const laneIds = current.draft.configuration.lanes.map(
            () => `lane_${randomBytes(16).toString('hex')}`
          );
          ...
          db.prepare(
            `INSERT INTO hosted_team_configuration_promotions
          (operation_id, workspace_id, team_id, actor_id, deployment_id, idempotency_key, record_json)
          VALUES (?, ?, ?, ?, ?, ?, ?)`
          ).run(
  ...
  lookup(value: unknown): HostedPromotionRecord | null {
    ...
    const token =
      'operationId' in input.reference
    ...
    return db.transaction(() => {
      const row = db
        ...
        .get(input.workspaceId, input.teamId, input.actorId, input.deploymentId, token) as
        | Row
        | undefined;
      if (!row) return null;
...
function readRecord(db: Database, row: Row): HostedPromotionRecord {
  if (Buffer.byteLength(row.record_json) > 2 * 1024 * 1024)
    throw new Error('promotion-record-too-large');
  ...
  const publication = new TeamDraftPublicationStorageOps(
    () => db,
    () => 0
  ).read(recordScope(record));
  if (
    !publication ||
    publication.state === 'tombstoned' ||
  ...
  const createKeys = db
    .prepare(
      `SELECT initial_revision FROM hosted_team_configuration_create_keys
    WHERE workspace_id = ? AND team_id = ?`
    )
    .all(record.workspaceId, record.teamId) as { initial_revision: string }[];
  ...
  if (
    ...
    !('schemaVersion' in roster) ||
    roster.schemaVersion !== 1 ||
    !('members' in roster) ||
    !('configuration' in roster)
  ) {
  ...
  if (
    draft.workspaceId !== record.workspaceId ||
    draft.teamId !== record.teamId ||
