...
import { isInternalStorageMutation } from './internalStorageMutationClassification';
import {
...
export class InternalStorageWorkerCore {
  ...
  private readonly hostedRunReservationOps = new HostedLifecycleRunReservationOps(
    () => this.open().db,
    () => (this.options.now?.() ?? new Date()).getTime(),
    () => this.options.promotionCommitAuthority
  );
