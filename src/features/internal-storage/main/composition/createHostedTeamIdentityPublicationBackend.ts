...
async function databaseIdentity(databasePath: string): Promise<string> {
  ...
  for (const [index, part] of parts.entries()) {
    ...
    if (
      stat.isSymbolicLink() ||
      (file ? !stat.isFile() || stat.nlink !== 1n : !stat.isDirectory()) ||
...
export async function createHostedTeamIdentityPublicationBackend(input: {
  ...
  const draftConnectionIdentity = await input.drafts.initialize(
    databasePath === input.drafts.databasePath
  );
  ...
  const sharedWorker =
    databasePath === input.drafts.databasePath && initialCanonical === initialDraft;
  if (!sharedWorker && initialCanonical.split('/').at(-1) === initialDraft.split('/').at(-1)) {
    throw new Error('canonical-database-alias-refused');
  }
  const client = sharedWorker
    ? null
  ...
  const guard = async <T>(effect: () => Promise<T>): Promise<T> => {
    if (
      closed ||
      (await databaseIdentity(databasePath)) !== initialCanonical ||
    ...
    if ((await databaseIdentity(databasePath)) !== initialCanonical)
      throw new Error('canonical-database-replaced');
    return result;
  };
  try {
    await guard(async () => {
      if (
        client &&
        (await client.ping()).connectionFileIdentity !== initialCanonical.split('/').at(-1)
      ) {
  ...
  return Object.freeze({
    ...
    readSnapshot: () =>
      guard(() => {
        ...
        if (!input.drafts.captureIdentitySnapshot)
          throw new Error('canonical-snapshot-port-unavailable');
    ...
    gateway: Object.freeze({
      ...
      prepareReservedTeamAdoption: (value) =>
        guard(() => source.prepareReservedTeamAdoption(value)),
      recordTeamIdentityFilePublished: (value) =>
        guard(() => source.recordTeamIdentityFilePublished(value)),
