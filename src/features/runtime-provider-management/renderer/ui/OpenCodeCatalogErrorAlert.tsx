...
import { formatRuntimeProviderDiagnosticsCopyText, RuntimeProviderErrorAlert } from './RuntimeProviderErrorAlert';
...
export function formatOpenCodeCatalogReport(failures: readonly OpenCodeCatalogFailure[]): string {
  ...
  const index = current.map((failure, i) => [
  ...
  const heading = `OpenCode catalog diagnostics\n${index}\n${omitted ? `[truncated: ${omitted} failures omitted; copy individually]\n` : ''}`;
  const budget = Math.floor((MAX_REPORT - heading.length - 100) / Math.max(current.length, 1));
  return heading + current.map((failure, i) => `\n${i + 1}.\n${cleanRuntimeDiagnosticText(
    formatRuntimeProviderDiagnosticsCopyText(failure.message, failure.diagnostics), budget - 16
  )}`).join('\n');
}

export function OpenCodeCatalogErrorAlert({ failures }: { failures: readonly OpenCodeCatalogFailure[] }) {
  const { t } = useAppTranslation('common');
  if (!failures.length) return null;
  const report = formatOpenCodeCatalogReport(failures);
  return <div className="w-full min-w-0">
    <RuntimeProviderErrorAlert compact testId="opencode-catalog-error"
      message={t('providerModelBadges.checkFailed')} reportText={report} />
    {report.includes('[truncated') ? failures.map((failure, index) => (
      <RuntimeProviderErrorAlert key={index} compact testId={`opencode-catalog-error-${index}`}
