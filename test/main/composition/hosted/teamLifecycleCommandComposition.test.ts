...
describe('team lifecycle command hosted composition', () => {
  it.each(['idle', 'ambiguous'] as const)(
    ...
    async (terminalOutcome) => {
      const acl = await createAclServer({ terminalOutcome });
      ...
      try {
        ...
        expect(first.json()).toMatchObject({ kind: 'accepted', runId: RUN_ID });
        const stop = await app.inject({
          method: 'POST',
          url: '/api/hosted/v1/team-lifecycle/stop',
          payload: {
            ...launchBody(),
            runId: RUN_ID,
            commandId: 'lifecycle-command_composition-stop',
            idempotencyKey: 'idempotency_composition-stop',
          },
        });
        expect(stop.json()).toMatchObject({
          kind: terminalOutcome === 'idle' ? 'accepted' : 'operator_required',
        });
        expect(current.state()).toBe(terminalOutcome === 'idle' ? 'retired' : 'cleanup_pending');
        const authorizationsBeforeRetry = acl.requests.filter(
          (request) => request.operation === 'authorize'
        ).length;
        const next = await app.inject({
          ...
          payload: {
            ...
            commandId: 'lifecycle-command_composition-next',
            idempotencyKey: 'idempotency_composition-next',
          },
        });
        ...
        if (terminalOutcome === 'idle')
          expect(next.json()).toMatchObject({ runId: `run_${'e'.repeat(32)}` });
        else
          expect(acl.requests.filter((request) => request.operation === 'authorize')).toHaveLength(
            authorizationsBeforeRetry
          );
      } finally {
        composition.close();
