...
export function generationCoordinationStream() {
  ...
  const stream = createHostedCoordinationEventStream({
    ...
    storage: {
      ...
      coordinationEventPrune: async () => {
        throw new Error('unused prune');
      },
    },
    authorizer: {
  ...
  const request = () => ({
    headers: { origin: 'https://hosted.example', accept: 'text/event-stream' },
    query: { after: encodeReplayCursor({ ...metadata, eventSequence: 0 }) },
