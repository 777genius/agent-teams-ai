...
it('does not restore launch-state publication files from fileStats', async () => {
  ...
  try {
    ...
    await expect(fs.readFile(path.join(liveDir, 'launch-stopped.json'))).rejects.toMatchObject({
      code: 'ENOENT',
    });
  } finally {
    await fs.rm(root, { recursive: true, force: true });
