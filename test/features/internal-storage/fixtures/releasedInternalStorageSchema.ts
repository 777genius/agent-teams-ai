...
function restoreReleasedV33SchemaInTransaction(db: Database, reference: Database): void {
  expect(readSchemaVersion(db)).toBe(34);
  runHostedLifecycleRunAliasMigrationAdmission(db, true);
  expect(db.prepare('SELECT * FROM hosted_lifecycle_run_aliases').all()).toEqual([]);
  const names = new Set(HOSTED_LIFECYCLE_RUN_ALIAS_MIGRATION.statements.map((sql) =>
    /^CREATE (?:TABLE|TRIGGER) ([a-z_]+)/u.exec(sql)?.[1]));
  for (let index = 1; index <= 2; index += 1)
    names.add(`sqlite_autoindex_hosted_lifecycle_run_aliases_${index}`);
  const current = db.prepare(schema('main')).all() as { name: string }[];
  expect(current.filter(({ name }) => !names.has(name))).toEqual(reference.prepare(schema('main')).all());
  expect(db.prepare(schema('temp')).all()).toEqual(reference.prepare(schema('temp')).all());
  for (const sql of [...HOSTED_LIFECYCLE_RUN_ALIAS_MIGRATION.statements].reverse()) {
    const match = /^CREATE (TABLE|TRIGGER) ([a-z_]+)/u.exec(sql);
    if (!match) throw new Error('unexpected-run-alias-schema-object');
    db.exec(`DROP ${match[1]} main.${match[2]}`);
  }
