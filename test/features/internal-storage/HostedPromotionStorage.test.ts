  ...
  it.each([15, 28, 29])('retains a populated v30 promotion across restored marker %i and two opens', async (marker) => {
    ...
    try {
      ...
      for (let index = 0; index < 2; index += 1) {
        ...
        reopened.close();
        expect(writer.pragma('user_version', { simple: true })).toBe(35);
  ...
  it('migrates an unbound v30 promotion without inventing member IDs', async () => {
    ...
    try {
      ...
      for (const sql of [...HOSTED_LIFECYCLE_CURRENT_AUTHORITY_MIGRATION.statements].reverse()) {
        ...
        if (!match) throw new Error('current-lifecycle-schema-object-invalid');
        writer.exec(`DROP ${match[1] === 'UNIQUE INDEX' ? 'INDEX' : match[1]} main.${match[2]}`);
      }
