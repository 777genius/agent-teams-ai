...
// eslint-disable-next-line no-restricted-imports -- Server-only golden verifies the owner proof-last parser.
import {
...
function signedGolden(): string {
  const unsigned = JSON.stringify(hostedApprovalRuntimeGoldenRequest());
...
describe('hosted approval runtime owner wire', () => {
  ...
  it('accepts only canonical statuses and binds resolved scope to mount and restore', () => {
    const authority = parseHostedApprovalRuntimeWireAuthority(
      hostedApprovalRuntimeGoldenRequest().authority
    );
    ...
    expect(() =>
      parseHostedApprovalRuntimeResponsePayload(
        ...
        parseHostedApprovalDecisionDeliveryRequest({
          ...
          approvalGeneration: `generation_runtime-permission-${'6'.repeat(64)}`,
          decision: 'allow',
          partition: { teamId: authority.teamId, runId: `run_${'7'.repeat(32)}` },
          requestId: 'request_permission-1',
        }),
        authority
      )
  ...
  it.each([
  ...
  ])('enforces owner authorityGeneration grammar at %s', (_label, authorityGeneration, accepted) => {
    const authority = parseHostedApprovalRuntimeWireAuthority(
      hostedApprovalRuntimeGoldenRequest().authority
    );
    ...
    const parse = () => parseHostedApprovalRuntimeResponsePayload(
      'approval_ingress_authority_resolve',
      {
        status: 'resolved',
        scope: {
  ...
  it.each([
  ...
  ])('rejects %s at the raw socket boundary and invalidates the lease', async (_label, response) => {
    const golden = hostedApprovalRuntimeGoldenRequest();
    ...
    await expect(adapter.acknowledgePermissionApprovalIngressEffect(golden.payload)).rejects.toThrow();
