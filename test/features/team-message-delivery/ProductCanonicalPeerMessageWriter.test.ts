...
const effect: ProductCanonicalEffect = {
  ...
  recipient,
  taskRefs: [{ teamId: 'team_test', taskId: 'task_canonical',
...
function fixture() {
  ...
  const makeWriter = () => new ProductCanonicalPeerMessageWriter({
    ...
    taskRefForCanonicalTask: (task) => ({
      taskId: task.taskId, displayId: 'T-1', teamName: 'test-team',
    }),
...
describe('ProductCanonicalPeerMessageWriter', () => {
  it('commits one TeamInboxWriter-format row and immutable Owner delivery version', () => {
    const f = fixture();
    ...
    expect(messages).toEqual([{
      ...
      taskRefs: [{ taskId: 'task_canonical', displayId: 'T-1', teamName: 'test-team' }],
    }]);
