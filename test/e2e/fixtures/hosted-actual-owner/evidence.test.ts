...
function completeEvidence(): ActualOwnerEvidenceDocument {
  ...
  const processEvidence = (name: 'opencode' | 'orchestrator' | 'product', index: number) => ({
    ...
    sourceRef: name === 'opencode' ? 'd'.repeat(40) : String(index).repeat(40).slice(0, 40),
    uid: 1000,
  });
  ...
  return Object.freeze({
    ...
    postLedger: Object.freeze([
      ...
      ...REQUIRED_NEGATIVE_CASES.filter(
        ...
        Object.freeze({
          ...
          bodySha256: String(index + 4).repeat(64).slice(0, 64),
          conditional: true as const,
          decision: 'allow_once' as const,
    ...
    protectedEffectLedger: Object.freeze([
      Object.freeze({ approvalId: allowId, effectCount: 1, effectSha256: '4'.repeat(64), kind: 'allow' as const }),
      Object.freeze({ approvalId: denyId, effectCount: 0, effectSha256: null, kind: 'deny' as const }),
      Object.freeze({
        approvalId: ambiguousId,
        effectCount: 1,
...
describe('actual-owner evidence invariants', () => {
  ...
  it('rejects a missing negative and an automatic retry of an ambiguous effect', () => {
    ...
    const broken: ActualOwnerEvidenceDocument = {
      ...
      postLedger: Object.freeze([
        ...evidence.postLedger,
        { ...evidence.postLedger[2]!, sequence: 4, at: '2026-08-14T00:00:18.000Z' },
      ]),
      negatives: Object.freeze(evidence.negatives.filter((item) => item.case !== 'session_rotation')),
    };
    expect(validateActualOwnerCompletionEvidence(broken)).toEqual(
      expect.arrayContaining(['ambiguous_effect_retry_or_state_invalid', 'negative_session_rotation_invalid'])
    );
  });
