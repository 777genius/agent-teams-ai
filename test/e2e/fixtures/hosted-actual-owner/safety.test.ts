...
describe('candidate artifact pinning', () => {
  ...
  it('refuses an oversized OpenCode executable before digesting artifact bytes', async () => {
    const parent = await mkdtemp('/tmp/actual-owner-opencode-executable-size-');
    roots.push(parent);
    const executable = join(parent, 'opencode');
    const releaseManifest = join(parent, 'release-manifest.json');
    const sourceRef = 'e'.repeat(40);
    const oversizedExecutableBytes = 1024 * 1024 * 1024 + 1;
    await writeFile(executable, '', { mode: 0o700 });
    await truncate(executable, oversizedExecutableBytes);
    await chmod(executable, 0o500);
    await writeFile(
      releaseManifest,
      `${JSON.stringify({
        ...
        release: { sourceCommit: sourceRef },
        workflow: { name: 'release' },
        assets: [{ binarySha256: '0'.repeat(64), binarySize: oversizedExecutableBytes }],
      })}\n`,
      { mode: 0o600 }
    );
    ...
    ).rejects.toThrow(/opencode_executable_invalid/u);
  });
